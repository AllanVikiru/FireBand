      ...
      $.getJSON("../src/api/models/role/read_all.php", function (data) {
        ...
        $.each(data.records, function (key, val) {
            ...
            <div class="custom-control custom-radio custom-control-inline mb-5">
                <input class="custom-control-input" type="radio" name="example-inline-radios" id='`+ val.role_id + `' value='` + val.role_id + `'>
                <label class="custom-control-label" for='`+ val.role_id + `'>`+ val.role + `</label>
            </div>`;
        });
